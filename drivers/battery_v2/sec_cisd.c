...
{
	union power_supply_propval incur_val, chgcur_val, capcurr_val, vbat_val;
	struct cisd *pcisd = &battery->cisd;
	struct timespec now_ts;
	bool ret = false;
	static int prev_fullcap_rep;
#if defined(CONFIG_QH_ALGORITHM)
	union power_supply_propval qh_val, vfsoc_val, fullcap_nom_val;
	...
	if ((battery->status == POWER_SUPPLY_STATUS_CHARGING) ||
		...
		if (!pcisd->prev_qh_value &&
				battery->current_now > battery->pdata->cisd_qh_current_low_thr &&
				battery->current_avg > battery->pdata->cisd_qh_current_low_thr &&
				battery->current_now <= battery->pdata->cisd_qh_current_high_thr &&
				battery->current_avg <= battery->pdata->cisd_qh_current_high_thr &&
				vfsoc_val.intval >= battery->pdata->cisd_qh_vfsoc_thr) {
		...
		psy_do_property(battery->pdata->fuelgauge_name, get,
				POWER_SUPPLY_PROP_ENERGY_NOW, vfsoc_val);
		if (vfsoc_val.intval < battery->pdata->cisd_qh_vfsoc_thr)
			battery->qh_start = false;
#else
